...
namespace SalesManagementService.Application.Features.SalesInvoice.Commands
{
    public class CreateSalesInvoiceCommandValidator : AbstractValidator<CreateSalesInvoiceCommand>
    {
        public CreateSalesInvoiceCommandValidator()
        {
            ...
            RuleFor(x => x.SalesInvoiceDto.CreatedBy)
                .NotEmpty()
                .WithMessage("Created By is required.");

           

            RuleFor(x => x.SalesInvoiceDto.LineItems)
                .NotEmpty()
                .WithMessage("At least one line item is required.");

            RuleForEach(x => x.SalesInvoiceDto.LineItems)
                .SetValidator(new CreateSalesInvoiceLineItemDtoValidator());
        }
    }

    public class CreateSalesInvoiceLineItemDtoValidator : AbstractValidator<CreateSalesInvoiceLineItemDto>
    {
        public CreateSalesInvoiceLineItemDtoValidator()
        {
            ...
            RuleFor(x => x.CreatedBy)
                .NotEmpty()
                .WithMessage("Created By is required for line item.");
        }
    }
}
